...
async def executar_sessao(token, guild_id, tarefas, caminho_resultado, nome_sessao):
    ...
    async def handler(s):
        ...
        ok_count = len([r for r in s.results if r['status'] == 'OK'])
        err_count = len([r for r in s.results if r['status'] == 'ERRO'])
        fail_count = len([r for r in s.results if r['status'] == 'FALHA'])
