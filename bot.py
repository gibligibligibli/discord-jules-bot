...
async def executar_sessao(token, guild_id, tarefas, caminho_resultado, nome_sessao):
    ...
    async def handler(s):
        ...
        for i, task in enumerate(tarefas, 1):
            ...
            try:
                ...
                if len(s.results) > initial_results_len:
                    new_logs = s.results[initial_results_len:]
                    primary_log = new_logs[0]

                    # Update the editable message with the first primary outcome
                    await s.send_log_realtime(primary_log["status"], f"Tarefa {i} `{action}`: {primary_log['details']}", message_ref=msg_ref)

                    # Send subsequent atomic logs (like "Resultado: {...}") as separate new messages
                    for log in new_logs[1:]:
                        await s.send_log_realtime(log["status"], log["details"])
                else:
        ...
        for r in s.results:
            st = r.get("status")
            if st == "OK": ok_count += 1
            elif st == "ERRO": err_count += 1
            elif st == "FALHA": fail_count += 1
...
def main():
    ...
    nome_sessao = "".join(c for c in nome_sessao if c.isalnum() or c in ("-", "_")).strip()
    if not nome_sessao:
        nome_sessao = "sessao"
